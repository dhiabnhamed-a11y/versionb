...
export default async function AssetsPage() {
                ...
                <tbody>
                  {assets.map((asset) => (
                    <tr key={asset.id}>
                      ...
                      <td>{asset.department?.name || <span style={{ color: 'var(--text-light)' }}>&mdash;</span>}</td>
                      <td>
                      ...
                      <td>{asset.assignedUser?.name || <span style={{ color: 'var(--text-light)' }}>&mdash;</span>}</td>
  ...
  const serializedAssets = assets.map((a) => ({
    ...
    lastMaintenanceAt: a.lastMaintenanceAt?.toISOString() ?? null,
    nextMaintenanceAt: a.nextMaintenanceAt?.toISOString() ?? null,
  ...
  const serializedCategories = categories.map((c) => ({ id: c.id, name: c.name, assetType: c.assetType }))
  const serializedDepartments = departments.map((d) => ({ id: d.id, name: d.name }))
