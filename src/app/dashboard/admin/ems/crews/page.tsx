...
export default function CrewsPage() {
  ...
  if (loading) return <div style={{ padding: 24, color: '#64748b', fontSize: 13 }}>Loading crews...</div>
    ...
    <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: '#f1f5f9', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Users size={18} color="#8b5cf6" /> Crew Management
          </h1>
          <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>{crews.length} crews · {totalMembers} members · {availableMembers} available</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.05)', borderRadius: 6, padding: '6px 12px', border: '1px solid rgba(255,255,255,0.08)', flex: 1 }}>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search crews..." style={{ background: 'transparent', border: 'none', color: '#e2e8f0', fontSize: 13, outline: 'none', width: '100%' }} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: 10 }}>
        {filtered.length === 0 ? (
          <div style={{ gridColumn: '1 / -1', padding: 40, textAlign: 'center', color: '#475569' }}>
            ...
            <div style={{ fontSize: 13 }}>No crews found</div>
          </div>
        ) : (
          filtered.map((crew) => (
            <div key={crew.id} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '14px 16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                {crew.members.length === 0 && <div style={{ fontSize: 12, color: '#475569', textAlign: 'center', padding: 8 }}>No members assigned</div>}
              </div>
            </div>
          ))
        )}
