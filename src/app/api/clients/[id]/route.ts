...
import { serializeClient } from '@/modules/clients/dto'
import { logClientActivity } from '@/modules/clients/repository'
import {
  cleanText,
  normalizeClientStatus,
  nullableText,
} from '@/modules/clients/service'
import type { BillingSessionUser, ClientPatchBody } from '@/modules/clients/types'
...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  return withApiError(_req, async () => {
  ...
  return withApiError(
    ...
    async () => {
      ...
      try {
        ...
        logger.error('client.delete_failed', error, { clientId: id, companyId: user.companyId })
        return NextResponse.json(
          {
            error: 'Client could not be deleted.',
            detail: process.env.NODE_ENV === 'production' ? undefined : error instanceof Error ? error.message : undefined,
          },
          { status: 500 }
        )
      }
